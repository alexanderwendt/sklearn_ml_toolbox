...
def analyze_timegraph(source, features, image_save_directory):
    '''
    ...
    plot_autocorrelation(np.log(source['Close']), "OMXS30", mode="acf", lags=None, xlim=None, ylim=None, image_save_directory=image_save_directory)

    plot_autocorrelation(np.log(source['Close']), "OMXS30_700_first", mode="acf", lags=None, xlim=[0, 700], ylim=None,
                         image_save_directory=image_save_directory)

    plot_autocorrelation(np.log(source['Close']), "OMXS30", mode="pacf", lags=200, xlim=None, ylim=None,
                         image_save_directory=image_save_directory)

    plot_autocorrelation(np.log(source['Close']), "OMXS30_first_10", mode="pacf", lags=50, xlim=[0, 10], ylim=None,
                         image_save_directory=image_save_directory)


    plot_autocorrelation(features.MA200Norm, "OMXS30_MA200", mode="acf", lags=None, xlim=None, ylim=None,
                         image_save_directory=image_save_directory)

    plot_autocorrelation(features.MA200Norm, "OMXS30_MA200_first_200", mode="acf", lags=None, xlim=[0, 200], ylim=None,
                         image_save_directory=image_save_directory)

    plot_autocorrelation(features.MA200Norm, "OMXS30_MA200", mode="pacf", lags=200, xlim=None, ylim=None,
                         image_save_directory=image_save_directory)
    ...
    plot_autocorrelation(diff, "OMXS30_difference", mode="acf", lags=None, xlim=[0, 50], ylim=[-0.2, 0.2],
                         image_save_directory=image_save_directory)

    plot_autocorrelation(diff, "OMXS30_difference", mode="pacf", lags=100, xlim=[0, 50], ylim=[-0.2, 0.2],
                         image_save_directory=image_save_directory)
...
def analyse_features(features, y, class_labels, source, conf, image_save_directory):
    ...
    X_scaled, y_scaled = rescale_X(conf, features, y)
    ...
    plot_temporal_correlation_feature(X_scaled, conf, image_save_directory, source, y_scaled)
    ...
    y_train_scaled_subset = np.array(y[X_train_index_subset]).flatten()
    ...
    plot_parallel_coordinates(df_fv, cols, colors, comparison_name)
...
def plot_hierarchical_linkage(X_scaled, conf, image_save_directory):
    ...
    g = sns.clustermap(corr_matrix, row_linkage=linkage, col_linkage=linkage, row_cluster=True, col_cluster=True,
                       figsize=(30, 30), cmap=plt.get_cmap('coolwarm'))
...
def plot_temporal_correlation_feature(X_scaled, conf, image_save_directory, source, y_scaled):
    ...
    for col in X_scaled.columns:
        ...
        tmp = X_scaled.iloc[:, col_index].join(source).join(y_scaled).reset_index().set_index('Date').drop(
            columns=['id', 'Open', 'High', 'Low', 'Close'])
        # display(tmp)
        ...
        if image_save_directory:
            if not os.path.isdir(image_save_directory):
                os.makedirs(image_save_directory)
            plt.savefig(os.path.join(image_save_directory, conf['dataset_name'] + '_Temporal_Correlation_to_Outcome_{}'.format(col)), dpi=300)
...
def main():
    ...
    features, y, class_labels = load_features(conf)
